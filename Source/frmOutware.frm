...
Begin VB.Form frmOutware 
   Caption         =   "출고관리-수동(8030)"
   ClientHeight    =   9360
   ClientLeft      =   -165
   ClientTop       =   570
   ClientWidth     =   15180
   Icon            =   "frmOutware.frx":0000
   LockControls    =   -1  'True
   MDIChild        =   -1  'True
   ScaleHeight     =   9360
   ScaleWidth      =   15180
   ...
   Begin Threed.SSPanel pnlName 
      ...
      Begin VB.Label lblNotice 
         Caption         =   "3.환산은 한  셀 에 대해서만 처리합니다."
         Height          =   225
         Index           =   2
         Left            =   2310
         TabIndex        =   77
      ...
      End
      Begin VB.Label lblNotice 
         Caption         =   "1.수주단위와 상관없이 입력수량을 YD로 보고 M수량으로 변환합니다."
         Height          =   225
         Index           =   0
         Left            =   2310
         TabIndex        =   75
...
' 2012.11.23  오승욱,  계장, S_201212_태을염직_01 , 출고(수동) 에서 수주단위가 M인 경우 각Roll  수량 YD입력시 M로 계산되어 입력 되게
...
Public Sub LoadOutWare(ByVal OrderID As String, ByVal OutSeq As Integer)
    Dim II As Integer
    Me.Show
...
Private Sub cmdExcel_Click()
    ...
    If txtOrderID.Tag <> "" Then
        ...
        '2011.05.19, 요청번호: S_201105_태을염직_01 에 따른 추가
        Call MakeExcelPacking
...
ErrHandler:
    ...
    Call ErrorBox(Err.Number, "frmOutware.cmdFind_click", Err.Description)
End Sub
...
'        If grdOrder.TextMatrix(grdOrder.Row, 25) = "1" Then
'            MsgBox "검사자동패킹으로 출고작업한건입니다." & vbCrLf & "수동작업에서 수정할 수 없습니다", vbInformation
'            Exit Sub
'        End If
...
ErrHandler:
    Set oOutware = Nothing
    Set rs = Nothing
    Call ErrorBox(Err.Number, "frmOutware.txtOrderID_KeyPress", Err.Description)
End Sub
...
'Private Sub grdPacking_KeyPress(KeyAscii As Integer)
'    With grdPacking
''        If KeyAscii <> vbKeyReturn Or Len(.TextMatrix(.Row, .Col)) > 0 Or Len(.EditText) > 0 Then Exit Sub
...
'Private Sub grdPacking_KeyPressEdit(ByVal Row As Long, ByVal Col As Long, KeyAscii As Integer)
'    With grdPacking
'        If Row < .FixedRows Or Col < .FixedCols Then Exit Sub
'        If KeyAscii = vbKeyDown And .Row = .Rows - 1 Then
'            .Rows = .Rows + 1
'        End If
'
''        If .Col > 1 Then
''            If Not ((KeyAscii >= vbKey0 And KeyAscii <= vbKey9) Or KeyAscii = vbKeyBack Or KeyAscii = vbKeyDelete _
''                Or KeyAscii = vbKeyMultiply Or KeyAscii = 42 Or KeyAscii = 45 Or KeyAscii = vbKeySubtract Or KeyAscii = vbKeyReturn) Then
''                KeyAscii = 0
''            End If
''        End If
'    End With
...
'        End If
        ...
        If Col >= 3 Then
            If Len(.EditText) > 0 And CheckNum(.EditText) <> 0 Then
                ...
                'S_201212_태을염직_01 에 의한 추가
                If chkAutoCalc.Value = 1 Then           'YD->M 계산에 체크
''                    If txtUnit.Text = "0" Then          '수주단위 YD이면
                        nQty = nQty / 1.0936         'YD -> M으로 변화
''                    Else
''                        nQty = nQty * 1.0936         'M -> YD로 변화
''                    End If
                End If
                ...
                For i = 0 To nRoll - 1
                    If nCol = 13 Then
                       ...
                       .TextMatrix(Row + nRow, nCol) = CStr(Format(nQty, "#,###"))
                    Else
                        .TextMatrix(Row + nRow, nCol) = CStr(Format(nQty, "#,###"))
...
Private Sub InitGrid()
    ...
    With grdOrder
        ...
        .TextArray(4) = "작성일자":      .ColWidth(4) = 1000:    .ColAlignment(4) = flexAlignCenterCenter
   ...
   '     .RowHeightMin = 450
        ...
        For i = 0 To 9
            .TextArray(i + 3) = CStr(i + 1):  .ColWidth(i + 3) = 1000:     .ColAlignment(i + 3) = flexAlignCenterCenter
        Next i
...
ErrHandler:
    Call ErrorBox(Err.Number, "frmOutware.ShowData", Err.Description)
...
ErrHandler:
    ...
    Call ErrorBox(Err.Number, "frmOutware.FillGridPacking", Err.Description)
End Sub
...
ErrHandler:
    ...
    Call ErrorBox(Err.Number, "frmOutware.DeleteData", Err.Description)
End Function
...
ErrHandler:
    ...
    Call ErrorBox(Err.Number, "frmOutWare.MakeColorGridCombo", Err.Description)
End Sub
...
Private Sub PrintLinePrint()
    ...
    Dim i%, nRow%, nPage%, nLineFlag%, nLotCnt%, nLineCnt As Integer
    Dim JJ%
...
ErrHandler:
    ...
    Call ErrorBox(Err.Number, "frmOutware.PrintLinePrint", Err.Description)
End Sub
...
''        vCustom(3) = CheckNull(rs!Address1) & " " & CheckNull(rs!Address2)
    ...
    'oExcel.Application.Visible = True       '개발자 개발 Mode 에서는 풀고 Test 해도 됨, 사용자 적용시에는 막아야 함.
...
''        .Cells(12, 13) = Format(txtOutRealQty(1), "#,###")                  '생지마수- 길이  ***
        
        
        .Cells(12, 14) = Format(grdSum.TextMatrix(0, 1), "#,###")           '가공마수-절 ***
        If txtOutCustom.Tag = "0" Then
            .Cells(12, 17) = Format(grdSum.TextMatrix(0, 2), "#,###") & "Y" '가공마수길이
    ...
    If PlusMDI.PrintPreview = True Then         '인쇄미리보기 선택시 미리보기, 그외에는 바로 인쇄, S_201204_태을염직_02 의한 추가
        oExcel.WindowState = xlMaximized
