...
Begin VB.Form frmProcCostSaleReport 
   ...
   End
End
Attribute VB_Name = "frmProcCostSaleReport"
Attribute VB_GlobalNameSpace = False
Attribute VB_Creatable = False
Attribute VB_PredeclaredId = True
Attribute VB_Exposed = False
Option Explicit
Private Const REPORTFILE = "\Report\ProcCostSaleReport.rpt"     '---- 수출용 원자재 매도 확약서
Private Const REPORTFILE = "\Report\ProcCostSaleConfirm.rpt"    '---- 외화획득용원료 구매 승인신청서
...
'---- 인쇄 ---------
Sub DataPrn(ByVal sCustom As String)
    Dim oStuffIn As PlusLib2.CSubul
    Dim rs As ADODB.Recordset
    Dim sParam() As String
    Dim vCustom As Variant
    Dim nTotQty As Long, nTotPrice As Double
    
    
    vCustom = Split(sCustom, "|")
    Set oStuffIn = New PlusLib2.CSubul
    oStuffIn.Connection = g_adoCon
    oStuffIn.UserName = g_sUserName
    
    Set rs = oStuffIn.GetDeliveryReport(Left(MakeDate(DF_SHORT, dtpDate(0)), 6), 1, vCustom(1))
    
    nTotQty = 0: nTotPrice = 0

    Do Until rs.EOF
        nTotQty = nTotQty + SumQtyYDS
        nTotPrice = nTotPrice + AmountDollar
        rs.MoveNext
    Loop
    ...
    ReDim sParam(2)
    sParam(0) = vCustom(0)
    sParam(1) = SetCurrency(nTotQty) & "Y"
    sParam(2) = "US$" & SetCurrency(nTotPrice, 2)

    Call PrintReport(REPORTFILE, rs, sParam, PlusMDI.PrintPreview)
    Call PrintReport(REPORTFILE2, rs, sParam, PlusMDI.PrintPreview)
