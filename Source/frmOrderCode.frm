...
Begin VB.Form frmOrderCode 
   ...
   Begin TabDlg.SSTab tabForm 
      ...
      TabHeight       =   679
      TabCaption(0)   =   "원단폭 관리 "
      TabPicture(0)   =   "frmOrderCode.frx":000C
      Tab(0).ControlEnabled=   -1  'True
      Tab(0).ControlCount=   0
      TabCaption(1)   =   "가공구분 관리"
      TabPicture(1)   =   "frmOrderCode.frx":0028
      Tab(1).ControlEnabled=   0   'False
      Tab(1).ControlCount=   0
      TabCaption(2)   =   "레벨 구분"
      TabPicture(2)   =   "frmOrderCode.frx":0044
      Tab(2).ControlEnabled=   0   'False
      Tab(2).ControlCount=   0
      TabCaption(3)   =   "밴드 구분"
      TabPicture(3)   =   "frmOrderCode.frx":0060
      Tab(3).ControlEnabled=   0   'False
      Tab(3).ControlCount=   0
      TabCaption(4)   =   "주문 형태"
      TabPicture(4)   =   "frmOrderCode.frx":007C
      Tab(4).ControlEnabled=   0   'False
      Tab(4).ControlCount=   0
      TabCaption(5)   =   "필장구분"
      TabPicture(5)   =   "frmOrderCode.frx":0098
...
Private Sub tabform_Click(PreviousTab As Integer)
    ...
    Call PlusMDI.RunForm(1310 + (10 * tabForm.Tab))
...
Private Function CheckData() As Boolean
    ...
    If Len(Trim(txtName)) > 0 And m_sFlag = ID_ADDNEW Then
        With grdData
            For iLoop = .FixedRows To .Rows - .FixedRows
                If InStr(UCase(.TextArray(iLoop * .Cols + 2)), UCase(txtName)) = 0 Then
                    If MsgBox("같거나 비슷한 명칭이 있습니다." & vbCrLf & "그래도 등록하시겠습니까?", vbYesNo) = vbYes Then
                        CheckData = True
                        Exit Function
                    Else
                        CheckData = False
...
Private Function SaveData() As Boolean
    ...
    If tabForm.Tab = 1 Then
        NewCode.sCodeID = Format(txtCode, "000#")
    Else
    ...
    If m_sFlag = ID_ADDNEW Then
        If tabForm.Tab = 1 Then
            SaveData = oCode.AddNewCodeWork(NewCode)
        Else
            SaveData = oCode.AddNewCode(NewCode)
        End If
    ElseIf m_sFlag = ID_UPDATE Then
        If tabForm.Tab = 1 Then
            SaveData = oCode.UpdateCodeWork(NewCode)
        Else
